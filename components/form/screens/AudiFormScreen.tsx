...
const AuditFormScreen: React.FC<AuditFormScreenProps> = ({
  ...
  const [auditInfo, setAuditInfo] = useState<Stage>();
  ...
  const getFormStages = useCallback(async () => {
    try {
      ...
      setStages(data.audit_group);
      setAuditInfo(data.audit_info);
    } catch (error: any) {
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        >
          {!submitting ? (
            auditInfo?.questions.map((question: any) => (
        ...
        {stages.map((stage) => (
          ...
          >
            {!submitting ? (
              stage.questions.map((question: any) => (
                <View key={question.question_uuid}>
                  {renderQuestion(question)}
                </View>
              ))
